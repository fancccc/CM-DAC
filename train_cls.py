...
def main(args):
    ...
    for fold in range(0, 1):
        ...
        # from model.backbone.classify import Ablation_CLIP
        cls_model = CLIP(embed_dim=256,
                         model_depth=34,
        ...
        if args.phase == 'train':
            ...
            for epoch in range(args.epochs):
                ...
                for batch in progress_bar:
                    ...
                    # 生成文本特征
                    with torch.no_grad():
                        text_features = label2text2token(labels, labmap).to(device)

                    # 混合精度训练
                    with torch.cuda.amp.autocast():
                        logits_per_image, logits_per_text = cls_model(ct, clinical, text_features)
                ...
                all_class_text_features = label2text2token(all_class_labels, labmap).to(device)
                ...
                if accuracy >= best_acc:
                    best_acc = accuracy
                    best[fold] = {'fold': fold, 'Accuracy': accuracy, 'Precision': precision, 'Recall': recall,
                                  'F1 Score': f1, 'ROC-AUC (OvR)': roc_auc, 'Kappa': kappa}
