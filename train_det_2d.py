...
from data.dataloader import LUNA16Dataset, DataLoader, CSPL2dDetDataset
import torch
...
from model.backbone.loss import ComputeLoss, ComputeLoss2d
...
def main(args):
    ...
    det_model = Detection2d().to(device)
    optimizer = Adam(det_model.parameters(), lr=args.lr, weight_decay=0.001, betas=(0.9, 0.99))
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3, verbose=True)

    # Dataset and DataLoader
    root = args.root
    traindataset = CSPL2dDetDataset(root, os.path.join('ImageSets', 'Main', 'train.txt'))
    testdataset = CSPL2dDetDataset(root, os.path.join('ImageSets', 'Main', 'val.txt'))
    ...
    if args.phase == 'train':
        ...
        for epoch in tqdm(range(args.epochs), desc=f"Total"):
            ...
            for i, data in pbar_train:
                ...
                # if accelerator.is_local_main_process:
                metrics = mAP_2d(y.cpu().detach().numpy(), gts.cpu().numpy())
                pbar_train.set_postfix({"loss": loss.item(), 'ap': metrics['ap']})
            all_gts = torch.cat(all_gts, dim=0)
            all_preds = torch.cat(all_preds, dim=0)
            t_metrics = mAP_2d(all_preds.detach().cpu().numpy(), all_gts.detach().cpu().numpy())
    ...
    # Testing phase
        ...
        val_loss = 0.0
        with (torch.no_grad()):
            pbar_val = tqdm(enumerate(testloader), total=len(testloader))
            all_gts = []
            all_preds = []
            for i, data in pbar_val:
                img, gts = data['img'].to(device), data['relative_coord'].to(device)
            ...
            metrics = mAP_2d(all_preds.cpu().numpy(), all_gts.cpu().numpy())
            val_loss /= len(testloader)
            print(metrics, f'val_loss: {val_loss}')
